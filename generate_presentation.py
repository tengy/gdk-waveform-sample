...
# ── Colour palette ────────────────────────────────────────────────────────────
C_DARK_BLUE  = RGBColor(0x1A, 0x37, 0x6C)   # slide background / title bar
C_MID_BLUE   = RGBColor(0x2E, 0x6D, 0xB4)   # accent / table header
C_LIGHT_BLUE = RGBColor(0xD6, 0xE4, 0xF7)   # table row alt / highlight
C_WHITE      = RGBColor(0xFF, 0xFF, 0xFF)
C_DARK_TEXT  = RGBColor(0x1A, 0x1A, 0x2E)
C_ORANGE     = RGBColor(0xF0, 0x8C, 0x00)   # key-term highlight
...
# Title
add_textbox(slide, 1.5, 1.5, 10.3, 1.1,
...
# Chapter pills
chapters = [
    "Foundations", "Graphics", "Color & Video",
    "Animation", "Audio", "Typography", "Design", "Accessibility"
]
pill_w, pill_h = 1.4, 0.36
pill_gap = 0.12
...
for i, ch in enumerate(chapters):
    ...
    add_rect(slide, px, 3.65, pill_w, pill_h,
             fill_rgb=C_MID_BLUE,
             line_rgb=C_LIGHT_BLUE, line_width_pt=0.5)
    add_textbox(slide, px, 3.65, pill_w, pill_h,
                ch, font_size=10, bold=True, color=C_WHITE,
                align=PP_ALIGN.CENTER)

# Author
add_textbox(slide, 1.5, 4.3, 10.3, 0.45,
            "Dr. Ahmed H. Eltengy",
...
add_textbox(slide, 0.2, 7.12, 12.9, 0.3,
            ...
            font_size=9, color=RGBColor(0xAA, 0xC4, 0xE8),
            align=PP_ALIGN.CENTER)


# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 2 — Chapter 1: Introduction to Multimedia
# ══════════════════════════════════════════════════════════════════════════════
...
# Left column: definition + components
add_key_box(slide, 0.3, 1.85, 5.8, 1.0,
            "KEY DEFINITION",
            "Multimedia: Integration of two or more media types (text, images, audio,
            video, animation) presented through a digital, typically interactive, system.")
...
# Right column: history timeline table
...
add_bullet_box(slide, 6.3, 4.75, 6.8, 1.35,
               ...
               title="⚙️  Production Pipeline & Digitization",
               bg_color=RGBColor(0xEB, 0xF2, 0xFB), font_size=11)


# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 3 — Chapter 2: Digital Media & Vector Graphics
# ══════════════════════════════════════════════════════════════════════════════
slide = add_slide_base(
...
# Vector vs Bitmap comparison cards
for lx, title_txt, items, bg in [
    ...
    (6.8, "BITMAP / RASTER", [
        ...
        "Described by Width × Height × Bit-depth",
        "Common sizes: HD 1280×720, 4K 3840×2160",
...
# Geometric transformations strip
...
tforms = [
    ...
    ("Rotation", "Turns by angle θ around\na center point"),
...
for i, (name, desc) in enumerate(tforms):
    ...
    add_rect(slide, cx, 5.42, card_w, 1.55,
             ...
             line_rgb=C_MID_BLUE, line_width_pt=0.5)
...
# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 4 — Chapter 3: Graphics & Image Processing
# ══════════════════════════════════════════════════════════════════════════════
slide = add_slide_base(
...
add_textbox(slide, 4.5, 1.85, 8.6, 0.3,
            ...
            font_size=12, bold=True, color=C_MID_BLUE)
...
add_simple_table(
    ...
    rows=[
        ...
        ["48-bit", "Trillions (16×3)","6 B",  "RAW professional photography"],
    ],
    col_widths=[1.6, 2.4, 1.5, 3.1],
    header_font=10, body_font=9
)
...
add_simple_table(
    ...
    col_widths=[1.1, 1.9, 3.7],
    header_font=10, body_font=9
)


# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 5 — Chapter 4: Color Science & Video
# ══════════════════════════════════════════════════════════════════════════════
slide = add_slide_base(
...
add_bullet_box(slide, 0.3, 4.7, 6.8, 1.45,
               [
                   ...
                   "4:2:2 — Cb/Cr at ½ horizontal → 33% saving (broadcast)",
                   "4:2:0 — Cb/Cr at ½ H+V → 50% saving (H.264, streaming)",
               ],
               title="📊  YCbCr & Chroma Subsampling",
               bg_color=RGBColor(0xEB, 0xF2, 0xFB), font_size=11)

# NLE / compositing key points
add_bullet_box(slide, 7.3, 4.7, 5.8, 1.45,
               ...
               title="✂️  Editing & Compositing",
               bg_color=RGBColor(0xEB, 0xF2, 0xFB), font_size=11)


# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 6 — Chapter 5: Animation Fundamentals
# ══════════════════════════════════════════════════════════════════════════════
slide = add_slide_base(
...
add_bullet_box(slide, 0.3, 1.85, 4.0, 2.55,
               [
                   ...
                   "10 dB = ×10 intensity, ~×2 perceived loud",
...
add_bullet_box(slide, 0.3, 4.55, 6.2, 1.55,
               ...
               title="🎛️  Signal Processing & Effects",
               bg_color=RGBColor(0xFFF3D0), font_size=11)
...
# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 8 — Chapter 7: Text, Typography & The Web
# ══════════════════════════════════════════════════════════════════════════════
slide = add_slide_base(
...
# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 9 — Chapters 8 & 9: Visual Design & Interactivity
# ══════════════════════════════════════════════════════════════════════════════
slide = add_slide_base(
...
add_bullet_box(slide, 6.5, 1.85, 6.55, 2.55,
               [
                   ...
                   "Rule of thirds: divide canvas 3×3, place focal points",
...
add_bullet_box(slide, 6.5, 4.5, 6.55, 1.65,
               ...
               title="🖱️  Interactivity & Storytelling",
               bg_color=RGBColor(0xFFF3D0), font_size=11)


# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 10 — Chapter 10: Accessibility & Key Takeaways
# ══════════════════════════════════════════════════════════════════════════════
slide = add_slide_base(
...
add_bullet_box(slide, 0.3, 3.85, 6.0, 2.3,
               [
                   "Text alternatives: <alt> for all meaningful images",
...
for i, (ch, desc) in enumerate(summary):
    row, col = divmod(i, 3)
    cx2 = 6.5 + col * (cw2 + 0.07)
    cy2 = 3.95 + row * (ch2 + 0.06)
    add_rect(slide, cx2, cy2, cw2, ch2,
             fill_rgb=C_LIGHT_BLUE if (row + col) % 2 == 0 else C_WHITE,
             line_rgb=C_MID_BLUE, line_width_pt=0.4)
    add_textbox(slide, cx2 + 0.1, cy2 + 0.04, cw2 - 0.2, 0.3,
                ch, font_size=9, bold=True, color=C_DARK_BLUE)
    add_textbox(slide, cx2 + 0.1, cy2 + 0.36, cw2 - 0.2, 0.35,
                desc, font_size=8, color=C_DARK_TEXT)
